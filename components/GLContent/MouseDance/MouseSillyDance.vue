...
import { MeshMatcapMaterial, TextureLoader, DoubleSide, PointLight, Clock, AnimationMixer } from 'three'
...
export default {
  ...
  methods: {
    configModel ({ model, mats }) {
      ...
      // var amb = new AmbientLight(0xffffff, 1); // soft white light
      // this.o3d.add(amb)
    ...
    async loadStuff () {
      ...
      let all = await Promise.all([
        new Promise((resolve) => {
          ...
          loaderModel.load(require('file-loader!./mouse/mousesillydance.glb'), (v) => {
            resolve(v)
          })
        }),
        // new Promise((resolve) => {
