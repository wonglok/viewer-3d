  ...
  methods: {
    configModel ({ model, mats }) {
      ...
      model.scene.traverse((item) => {
        ...
        if (item.isMesh && item.name === 'Mesh.002_0') {
          // metal
        ...
        if (item.isMesh && item.name === 'Mesh.002_1') {
          // Cloth
    ...
    async loadStuff () {
      ...
      let all = await Promise.all([
        new Promise((resolve) => {
          ...
          let url = require('file-loader!./swat/mma-kick.glb')
          ...
          // // eslint-disable-next-line
          // loaderModel.load(require('file-loader!./swat/mma-kick.glb'), (v) => {
          //   resolve(v)
          // }, (v) => {
          //   // this.loaderAPI.updateProgress(v.loaded / v.total)
          // })
