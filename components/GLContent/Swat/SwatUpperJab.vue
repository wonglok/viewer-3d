  ...
  methods: {
    configModel ({ model, mats }) {
      ...
      model.scene.traverse((item) => {
        ...
        if (item.isMesh && item.name === 'Mesh.004_0') {
          // metal
        ...
        if (item.isMesh && item.name === 'Mesh.004_1') {
          // Cloth
    ...
    async loadStuff () {
      ...
      let all = await Promise.all([
        new Promise((resolve) => {
          ...
          // loaderModel.load(require('file-loader!./swat/upper-jab.glb'), (v) => {
          //   resolve(v)
          // }, (v) => {
          //   // this.loaderAPI.updateProgress(v.loaded / v.total)
          // })

          let url = require('file-loader!./swat/upper-jab.glb')
